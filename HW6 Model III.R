...
fev.dat3 =list(n=nrow(fevstar.dat), Smoke=Smoke, Age=Age, FEV=FEV)
bugs.data(data=fev.dat3, data.file="FEV-Regression-model3.txt")
...
#INITS - Chain 1
inits1 =list(tau=1.0, beta = c(0.0, 0.0, 1.0))
#INITS - Chain 2
inits2 =list(tau= 5.0, beta = c(1.0, 2.0, -1.0))
#INITS - Chain 3
inits3 =list(tau=10.0, beta = c(0.0, -2.0, 0.0))


inits=list(inits1, inits2, inits3)


model.III.with.coda= bugs(data=fev.dat3,inits,OpenBUGS.pgm=OpenBUGS.pgm,model.file = "Model-III.txt",
...
library(coda)
library(lattice)
out.coda = read.bugs(model.III.with.coda)
class(out.coda)
str(out.coda)
...
# 
model.III=bugs(data=fev.dat3,inits = inits,OpenBUGS.pgm=OpenBUGS.pgm,model.file = "Model-III.txt",
                    n.burnin=500,parameters=c("post.prob.beta2","FEV15","beta"),
                    n.chains = 3,n.iter=5500, bugs.seed = 12, codaPkg = FALSE, useWINE = TRUE)
# 
print(model.III,digits=4)
quartz(width=4,height=4,pointsize=8)
plot(model.III)
